...
export class AutoGenerator {
  ...
  private addTable(table: string) {
    ...
    str += this.options.colsAsObject ? '...attributes\n' : tableDefinition;
    this.attributes = `{\n${tableDefinition}\n}`;
  ...
  private addField(table: string, field: string): string {
    ...
    const space = this.space;
    const addSpace = this.options.colsAsObject ? 1 : 0;
    ...
    fieldAttrs.forEach((attr) => {
      ...
      if (isSerialKey && !wroteAutoIncrement) {
        str += space[3 + addSpace] + 'autoIncrement: true,\n';
        ...
        if (
          ...
          str += space[3 + addSpace] + 'autoIncrementIdentity: true,\n';
        }
      ...
      if (attr === 'foreignKey') {
        ...
        if (fieldObj[attr] === true && (!_.has(fieldObj, 'foreignKey') || !!fieldObj.foreignKey.isPrimaryKey)) {
          str += space[3 + addSpace] + 'primaryKey: true';
        } else {
          return true;
        }
        ...
        str += space[3 + addSpace] + attr + ': ' + fieldObj[attr];
        ...
        if (_.isString(defaultVal)) {
          ...
          if (field_type.match(/^(json)/)) {
            const parsed = JSON.parse(defaultVal);
            if (Array.isArray(parsed)) {
              // defaultVal = defaultVal;
            } else {
              defaultVal = JSON.stringify(JSON.parse(defaultVal), null, 4);
            }
          } else {
            defaultVal = this.escapeSpecial(defaultVal);
          }

          // defaultVal = field_type.match(/^(json)/)
          //   ? JSON.stringify(JSON.parse(defaultVal), null, 4)
          //   : this.escapeSpecial(defaultVal);
        ...
        str += space[3 + addSpace] + attr + ': ' + val_text;
